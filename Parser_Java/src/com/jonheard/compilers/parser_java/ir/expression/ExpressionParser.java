...
public class ExpressionParser {
  public static BaseIrType parseExpressionStatment(Parser parser) {
    ...
    switch (result.getExpressionType()) {
      ...
      default:
        Logger.error("not a statement", parser.getSource(), next.getRow(), next.getColumn());
        break;
    }
  ...
  private static Expression tryAssignment(Parser parser) {
    ...
    if (parser.passTokenIfType(TokenType.EQUAL))
      return new Expression(ExpressionType.ASSIGMENT, next, lhs, tryAssignment(parser));
    if (parser.passTokenIfType(TokenType.PLUS_EQUAL))
      return new Expression(ExpressionType.ASSIGMENT_ADD, next, lhs, tryAssignment(parser));
    if (parser.passTokenIfType(TokenType.DASH_EQUAL))
      return new Expression(ExpressionType.ASSIGMENT_SUB, next, lhs, tryAssignment(parser));
    if (parser.passTokenIfType(TokenType.STAR_EQUAL))
      return new Expression(ExpressionType.ASSIGMENT_MUL, next, lhs, tryAssignment(parser));
    if (parser.passTokenIfType(TokenType.SLASH_EQUAL))
      return new Expression(ExpressionType.ASSIGNMENT_DIV, next, lhs, tryAssignment(parser));
    if (parser.passTokenIfType(TokenType.PERCENT_EQUAL))
      return new Expression(ExpressionType.ASSIGNMENT_MOD, next, lhs, tryAssignment(parser));
    else
      return lhs;
  }
  ...
  private static Expression tryConditional(Parser parser) {
    ...
    if (parser.passTokenIfType(TokenType.QUESTION)) {
      ...
      lhs = new Expression(ExpressionType.CONDITIONAL, next, lhs, consequent, alternative);
    }
    return lhs;
  }
  ...
  private static Expression tryLogical(Parser parser) {
    ...
    while (more) {
      if (parser.passTokenIfType(TokenType.AND))
        lhs = new Expression(ExpressionType.LOGICAL_AND, next, lhs, tryEquality(parser));
      else if (parser.passTokenIfType(TokenType.PIPE))
        lhs = new Expression(ExpressionType.LOGICAL_OR, next, lhs, tryEquality(parser));
      else
        more = false;
    }
    return lhs;
  ...
  private static Expression tryEquality(Parser parser) {
    ...
    while (more) {
      if (parser.passTokenIfType(TokenType.EQUAL_EQUAL))
        lhs = new Expression(ExpressionType.EQUALITY, next, lhs, tryRelational(parser));
      else
        more = false;
    }
    return lhs;
  ...
  private static Expression tryRelational(Parser parser) {
    ...
    if (parser.passTokenIfType(TokenType.RIGHT_TRI))
      lhs = new Expression(ExpressionType.GREATER, next, lhs, tryAdditive(parser));
    if (parser.passTokenIfType(TokenType.LEFT_TRI))
      lhs = new Expression(ExpressionType.LESS, next, lhs, tryAdditive(parser));
    if (parser.passTokenIfType(TokenType.RIGHT_TRI_EQUAL))
      lhs = new Expression(ExpressionType.GREATER_OR_EQUAL, next, lhs, tryAdditive(parser));
    if (parser.passTokenIfType(TokenType.LEFT_TRI_EQUAL))
      lhs = new Expression(ExpressionType.LESS_OR_EQUAL, next, lhs, tryAdditive(parser));
    return lhs;
  }
  ...
  private static Expression tryAdditive(Parser parser) {
    ...
    while (more) {
      if (parser.passTokenIfType(TokenType.PLUS))
        lhs = new Expression(ExpressionType.ADD, next, lhs, tryMultiplicative(parser));
      else if (parser.passTokenIfType(TokenType.DASH))
        lhs = new Expression(ExpressionType.SUB, next, lhs, tryMultiplicative(parser));
      else
        more = false;
    }
    return lhs;
  ...
  private static Expression tryMultiplicative(Parser parser) {
    ...
    while (more) {
      if (parser.passTokenIfType(TokenType.STAR))
        lhs = new Expression(ExpressionType.MUL, next, lhs, tryUnary(parser));
      else if (parser.passTokenIfType(TokenType.SLASH))
        lhs = new Expression(ExpressionType.DIV, next, lhs, tryUnary(parser));
      else if (parser.passTokenIfType(TokenType.PERCENT))
        lhs = new Expression(ExpressionType.MOD, next, lhs, tryUnary(parser));
      else
        more = false;
    }
    return lhs;
  ...
  private static Expression tryUnary(Parser parser) {
    ...
    if (parser.passTokenIfType(TokenType.PLUS_PLUS))
      return new Expression(ExpressionType.PRE_INCREMENT, next, tryUnary(parser));
    else if (parser.passTokenIfType(TokenType.DASH_DASH))
      return new Expression(ExpressionType.PRE_DECREMENT, next, tryUnary(parser));
    else if (parser.passTokenIfType(TokenType.PLUS))
      return new Expression(ExpressionType.POSITIVE, next, tryUnary(parser));
    else if (parser.passTokenIfType(TokenType.DASH))
      return new Expression(ExpressionType.NEGATIVE, next, tryUnary(parser));
    else
  ...
  private static Expression trySimpleUnary(Parser parser) {
    ...
    if (parser.passTokenIfType(TokenType.EXCLAIM))
      return new Expression(ExpressionType.LOGICAL_NOT, next, tryUnary(parser));
  ...
  private static Expression tryPrimary(Parser parser) {
    ...
    // this
      ...
      if (parser.passTokenIfType(TokenType.LEFT_PAREN)) {
        result = new ThisConstructor(next, new List_Expression(parser));
        parser.requireTokenToBeOfType(TokenType.RIGHT_PAREN);
      } else {
    ...
    // super
      ...
      if (parser.passTokenIfType(TokenType.DOT)) {
        ...
        parser.requireTokenToBeOfType(TokenType.LEFT_PAREN);
        result = new SuperConstructor(next, new List_Expression(parser));
        parser.requireTokenToBeOfType(TokenType.RIGHT_PAREN);
      }
    }
    // new
    ...
    // literal
      ...
      if (literalTokenType != TokenType.INTEGER && literalTokenType != TokenType.LONG
          && literalTokenType != TokenType.FLOAT && literalTokenType != TokenType.DOUBLE
          && literalTokenType != TokenType.CHAR && literalTokenType != TokenType.STRING
          && literalTokenType != TokenType._TRUE && literalTokenType != TokenType._FALSE
          && literalTokenType != TokenType._NULL) {
        Logger.error("illegal start of expression", parser.getSource().getFilename(),
            literalToken.getRow(), literalToken.getColumn(),
